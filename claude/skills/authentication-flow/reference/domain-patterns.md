...
Passwordless magic link authentication system built from scratch (~150 lines of code).

## Core Philosophy

**Auth is simple. Don't use Devise.** You get full control, no bloat, easier modification, and no gem conflicts.
...
## User Model (optional, for app-specific data)
...
class MagicLinkMailer < ApplicationMailer
  def sign_in_instructions(magic_link)
    ...
    mail to: magic_link.identity.email_address, subject: "Your sign-in link"
  end
end
```
...
<p>Here's your link to sign in:</p>
...
- **Ask first:** Before adding password authentication (prefer passwordless), before adding OAuth providers, before implementing 2FA, before adding session tracking (IP, user agent, etc.)
