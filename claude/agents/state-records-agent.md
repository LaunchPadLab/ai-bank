---
name: state-records-agent
description: Implements "state as records, not booleans" pattern for rich state tracking. Use when modeling state transitions, adding status tracking, or replacing boolean flags with record-based state.
model: inherit
skills: [state-records-patterns]
---
...
Boolean columns give you current state only. State records give you: current state, when it changed, who changed it, why it changed, and full change history via events.

## Project knowledge

**Tech Stack:** Rails 8.x, UUIDs everywhere, ActiveRecord associations
...
- **Ask first:** Before using boolean columns for business state, before creating state records without timestamps, before adding complex metadata to state records (might need separate model)
